<template>
  <div class="app-container">
    ...
    >
      <el-table-column label="订单号" prop="id" align="center" width="100px">
        <template slot-scope="{row}">
          <span>{{ row.order_no }}</span>
        </template>
      </el-table-column>
      <el-table-column label="就诊医院" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.hospital_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="科室" width="150px">
        <template slot-scope="{row}">
          <span>{{ row.dept }}</span>
          <!--                    <el-tag>{{ row.type | typeFilter }}</el-tag>-->
        </template>
      </el-table-column>
      <el-table-column label="就诊医生" min-width="150px" align="flex-start">
        <template slot-scope="{row}">
      ...
      <el-table-column label="就诊卡号" min-width="150px" align="flex-start">
        <template slot-scope="{row}">
          <span>{{ row.p_card_info.p_card }}</span>
        </template>
      </el-table-column>
      <el-table-column label="电话" min-width="150px" align="flex-start">
        <template slot-scope="{row}">
          <span>{{ row.p_card_info.mobile }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="订单状态" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.status==2?'已结束':'问诊中' }}</span>
        </template>
      </el-table-column>
      <el-table-column
        v-if="0"
...
import { getHospitalList, getRecordsList } from '@/api/kangying-https'
...
export default {
    ...
    methods: {
        ...
        getList() {
            ...
            getRecordsList(this.listQuery).then(res => {
